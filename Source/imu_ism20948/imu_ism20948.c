...
static void powerup_imu(void){
    ...
    spi_write_blocking(REG_BANK_SEL_ADD, (0x00 << REG_BANK_SEL_USER_BANK_Pos));     
//    spi_read_async(REG_BANK_SEL_ADD, 1);    // ожидаем 0x00
//    while(cur_spi_state != DATA_READY) __NOP();
//    transmit_byte_usart3_debug(spi_byte_read);
...
static void configure_gyro(void){
    ...
    spi_write_blocking(REG_BANK_SEL_ADD, (0x02 << REG_BANK_SEL_USER_BANK_Pos));
    spi_read_async(REG_BANK_SEL_ADD, 1);    // ожидаем 0x20
    while(cur_spi_state != DATA_READY) __NOP();
    transmit_byte_usart3_debug(spi_byte_read);
    ...
    spi_write_blocking(GYRO_CONFIG_1_ADD, gyro_cfg1);
...
static void configure_accel(void){
    ...
    spi_write_blocking(ACCEL_SMPLRT_DIV_1_ADD, 0x00);
    spi_write_blocking(ACCEL_SMPLRT_DIV_2_ADD, 0x00);
	...
	spi_write_blocking(ACCEL_CONFIG_ADD, acc_cfg);
...
static void configure_magnetometer(void){
    ...
    //set user bank 3
    spi_write_blocking(REG_BANK_SEL_ADD, (0x03 << REG_BANK_SEL_USER_BANK_Pos));
    ...
    spi_write_blocking(I2C_MST_CTRL_ADD, (7U << I2C_MST_CTRL_I2C_MST_CLK_Pos));
    
     /* -------- Magnetometer internal setting configuration ---------------- */
    //set transfer for write +  mag i2c address  
    spi_write_blocking(I2C_SLV0_ADDR, 
                            (0 << I2C_SLV0_ADDR_I2C_SLV0_RNW_Pos) |
                            (MAG_I2C_ADD << I2C_SLV0_ADDR_I2C_ID_0_Pos));
    
    //Reset magnitometer (continous mode 4)
    spi_write_blocking(I2C_SLV0_REG_ADD, MAG_CNTL3_ADD); //select add of reg to write data to
    spi_write_blocking(I2C_SLV0_DO_ADD, (1 << MAG_CNTL3_SRST_Pos)); //reset magnetometer
    //tranfer len = 1 byte + enable i2c write
    spi_write_blocking(I2C_SLV0_CTRL_ADD, 
                            (1U << I2C_SLV0_CTRL_I2C_SLV0_LENG_Pos)|
                            (1U << I2C_SLV0_CTRL_I2C_SLV0_EN_Pos));
    delay_ms(1);
    spi_write_blocking(I2C_SLV0_CTRL_ADD, (0U << I2C_SLV0_CTRL_I2C_SLV0_EN_Pos));
    
    // Set mag freq to 100 Hz (continous mode 4)
    spi_write_blocking(I2C_SLV0_REG_ADD, MAG_CNTL2_ADD);
    spi_write_blocking(I2C_SLV0_DO_ADD, (1 << MAG_CNTL2_MODE_3_Pos)); 
    spi_write_blocking(I2C_SLV0_CTRL_ADD, 
                            (1U << I2C_SLV0_CTRL_I2C_SLV0_LENG_Pos)|
                            (1U << I2C_SLV0_CTRL_I2C_SLV0_EN_Pos));
    delay_ms(1);
    ...
    spi_write_blocking(I2C_SLV0_REG_ADD, MAG_ST1_ADD);
    ...
    spi_write_blocking(I2C_SLV0_CTRL_ADD, slave0_cntrl_data);
